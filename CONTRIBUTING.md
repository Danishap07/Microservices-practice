...
WIP branches are named `wip/<feature-name>`.
...
6. **Error handling** — throw `AppError` subclasses from `@microkit/shared` for expected errors. The global error handler middleware catches everything.
...
4. If it needs shared code, add a dependency on `@microkit/shared`.
